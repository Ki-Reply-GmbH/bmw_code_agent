...
class MergeGitHandler(GitHandler):
    ...
    def git_actions(self, file_paths, commit_msg):
        """
        ...
        """
        GitHandler._repo.git.add(file_paths)
        GitHandler._repo.git.commit("-m", commit_msg)
        GitHandler._repo.git.push("--set-upstream", "origin", GitHandler._repo.active_branch.name)
