...
def main(json_deployment: str, text_deployment: str, git_repo: str,
    pr_number: str):
    ...
    owner = 'GCDM'
    repo = git_repo
    source_branch, target_branch = get_pr_branches(git_user, token, owner,
        git_repo, pr_number)
    file_list = get_changed_files(token=token, git_owner=owner, git_repo=
        repo, pr_number=pr_number)
    pr_gi = PRGitHandler(pr_number)
    gi = GitHandler()
    gi.initialize(source_branch, target_branch, git_user, owner, token,
        repo, pr_number)
    gi.set_credentials(email='optima-coding-mentor@bmw.de', name=
        'Optima Coding Mentor')
    gi.clean_up()
    gi.clone()
    pr_gi.create_progress_bar(percentage=0, status=
        'Processing webhook information.')
    ...
    try:
        pr_gi.create_progress_bar(percentage=10, status=
            'Checking for merge conflicts.')
    except:
        pr_agent.report_error(
            'Pull Request Agent failed creating progress bar.')
    try:
        mgh = MergeGitHandler()
        mag = MergeAgent(gi._repo, json_model=json_deployment, text_model=
            text_deployment)
        ...
        for i, file_path in enumerate(mgh.get_unmerged_filepaths()):
            ...
            LOGGER.debug('Ai is solving the merge conflict in %s...', mgh.
                get_unmerged_filepaths()[i])
            resp = mag.solve_merge_conflict()
        LOGGER.debug('Committing changes...')
        gi.write_responses(mag.get_file_paths(), mag.get_responses())
        mag.make_commit_msg()
        merge_commit_and_push = gi.commit_and_push(mag.get_file_paths(),
            mag.get_commit_msg())
    except:
    ...
    try:
        ...
        if merge_commit_and_push:
            pr_agent.set_memory('merge_agent', mag.get_file_paths(), mag.
                get_responses(), mag.get_commit_msg())
    except:
        pr_agent.report_error('Pull Request Agent failed to update memory.')
    try:
        """ Interaction with the Code Quality Agent """
        other_file_list = [file for file in updated_file_list if '.java' not in
            file]
        ...
        ja_lag = LintAgent(file_list=updated_file_list, directory=gi.
            get_tmp_path(), language='java', json_model=json_deployment,
        ...
        lint_commit_and_push = gi.commit_and_push(ja_lag.get_file_paths(),
            ja_lag.get_commit_msg())
        """
        py_lag = LintAgent(
        ...
        print(py_lag)
        """
        other_lag = LintAgent(file_list=other_file_list, directory=gi.
            get_tmp_path(), language='other', json_model=json_deployment,
            text_model=text_deployment)
        LOGGER.debug('Improving other code...')
        other_lag.improve_code(pr_gi, quant_java_files *
            progress_increment_per_file + 20, progress_increment_per_file)
        LOGGER.debug('Writing changes...')
        ...
        lint_commit_and_push = lint_commit_and_push or gi.commit_and_push(
            other_lag.get_file_paths(), other_lag.get_commit_msg())
    except:
        pr_agent.report_error('Code Quality Agent failed to improve code.')
        lint_commit_and_push = False
    pr_gi.create_progress_bar(percentage=90, status=
        'Updating the pull request comment.')
    try:
        """" Update the Pull Request Agent's memory """
        LOGGER.debug("Updating the Pull Request Agent's memory...")
        if lint_commit_and_push:
            pr_agent.set_memory('cq_agent', ja_lag.get_file_paths(), ja_lag
                .get_responses() + other_lag.get_responses(), ja_lag.
                get_commit_msg() + other_lag.get_commit_msg())
    ...
    try:
        ...
        pr_agent.report_error(
            'Pull Request Agent failed to update pull request.')
